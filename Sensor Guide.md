...
- SFM\_LP\_SET\_SIZE = the number of samples to be used when computing the mean. If there are not enough samples available then a record is still logged but with no values. This can not exceed the SFM\_LP\_BUFF\_LEN "Compiled Config" value.
